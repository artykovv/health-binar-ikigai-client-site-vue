      ...
      <div v-if="menuModalOpen && !hideMenu" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="closeMenuModal">
        <div class="w-full max-w-sm rounded-3xl p-8 shadow" :class="theme === 'dark' ? 'bg-[#282828] text-white' : 'bg-white text-zinc-900'" @click.stop>
          ...
          <div class="-mx-2">
            ...
            <router-link class="block w-full text-left px-2 py-4 text-lg border-b last:border-b-0 border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800" :to="{ name: 'purchases', params: { locale: locale } }" @click="closeMenuModal">{{ t('menu.purchases') }}</router-link>
